    ...
    for segmentation_mask, proposal in zip(segmentation_masks, proposals):
        ...
        if maskiou_on:
            ...
            for poly_ in segmentation_mask.instances:
                poly = np.array(poly_.polygons[0], dtype=np.float32)
            ...
            '''
            #type 2
            poly = segmentation_mask_for_maskratio.instances.polygons[0].polygons #.instances

            rle_for_fullarea = mask_util.frPyObjects([p.numpy() for p in poly], y2-y1, x2-x1)
            full_area = torch.tensor(mask_util.area(rle_for_fullarea).sum().astype(float))

            poly2 = cropped_mask.instances.polygons[0].polygons

            rle_for_box_area = mask_util.frPyObjects([p.numpy() for p in poly2], proposal[3]-proposal[1], proposal[2]-proposal[0])
...
def make_roi_mask_loss_evaluator_miou(cfg):
    """
    Different from the original maskrcnn, this file also calculates the iou of the predicted mask shape and groundtruth mask.
    """
    matcher = Matcher(
